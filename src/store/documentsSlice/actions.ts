...
export const generateShareCodeAction = async (documentId: string, permissions: ('EDIT' | 'READ')[]): Promise<string | null> => {
...
export const useShareCodeAction = async (documentId: string, shareCode: string): Promise<void> => {
  const response = await useShareCode(documentId, shareCode);
